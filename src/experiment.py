...
class FunduSAMFinetuner(nn.Module):
    def __init__(
        ...
        freeze_backbone_completely: bool = True,
        ...
        if not freeze_backbone_completely:
        ...
        if freeze_backbone_completely:
            # Maximum Memory Saving Mode
            for p in self.image_encoder.parameters():
                p.requires_grad = False
        elif freeze_encoders:
            # Standard Adapter Mode
            for p in self.image_encoder.parameters():
                p.requires_grad = False
...
class TrainConfig:
    ...
    freeze_backbone: bool = False  # New flag
...
class Trainer:
    def __init__(self, cfg: TrainConfig, dist: DistributedContext):
        ...
        self.model = FunduSAMFinetuner(
            sam_type=cfg.model, checkpoint=cfg.ckpt, 
            freeze_encoders=not cfg.unfreeze_encoders,
            freeze_backbone_completely=cfg.freeze_backbone,
        ).to(self.device)

        # Gradient Checkpointing Activation
        # If backbone is frozen, checkpointing is useless/harmful for performance
        if cfg.grad_checkpointing and not cfg.freeze_backbone:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--freeze-backbone", action="store_true",
                   help="Freeze entire encoder (including adapters). Only trains the head. Massive memory savings.")
