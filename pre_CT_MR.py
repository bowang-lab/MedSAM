...
remove_label_ids = [12] # remove deodenum since it is scattered in the image, which is hard to specify with the bounding box
...
for name in tqdm(names[:40]): # use the remaining 10 cases for validation
    ...
    if len(z_index)>0:
        ...
        sitk.WriteImage(img_roi_sitk, join(npy_path, prefix + gt_name.split(gt_name_suffix)[0]+'_img.nii.gz'))
        sitk.WriteImage(gt_roi_sitk, join(npy_path, prefix + gt_name.split(gt_name_suffix)[0]+'_gt.nii.gz'))
        ...
        for i in range(img_roi.shape[0]):
            ...
            np.save(join(npy_path, 'imgs', prefix + gt_name.split(gt_name_suffix)[0] + '-' + str(i).zfill(3) + '.npy'), resize_img_skimg_01)
            np.save(join(npy_path, 'gts', prefix + gt_name.split(gt_name_suffix)[0] + '-' + str(i).zfill(3) + '.npy'), resize_gt_skimg)
